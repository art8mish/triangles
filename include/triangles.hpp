...
private:
    ...
    std::pair<T, T> get_projection_segment_(const Vector<T> &axis) const {
        ...
        return std::pair<T, T>{std::min({proj1, proj2, proj3}),
                               std::max({proj1, proj2, proj3})};
    }

    static bool segments_intersect_(T segment1_a, T segment1_b, T segment2_a,
                                    T segment2_b) {
    ...
    bool projections_intersect_(const Triangle<T> &other,
                                const Vector<T> &axis) const {
    ...
    Triangle<T> to_2d_(const Vector<T> &u, const Vector<T> &v,
                       const Point<T> &p) const {
        return Triangle<T>{
            Point<T>{u.edot(Vector<T>{p, p1_}), v.edot(Vector<T>{p, p1_})},
            Point<T>{u.edot(Vector<T>{p, p2_}), v.edot(Vector<T>{p, p2_})},
            Point<T>{u.edot(Vector<T>{p, p3_}), v.edot(Vector<T>{p, p3_})},
        };
    }
    ...
    // return t for line equation p + vec(d) * t of Line(p2, p1) intersection
    // point
    ...
    std::pair<T, T>
    triangle_line_intersection_segment_(const Line<T> &line,
    ...
    bool triangle_point_intersection_(const Point<T> &point) const {
        ...
        T c = 1 - a - b;
        return (zero(a, eps_) || a > 0) && (zero(b, eps_) || b > 0) &&
               (zero(c, eps_) || c > 0);
    }
    ...
    bool segment_segment_intersection_(const Triangle<T> &other) const {
        ...
        // L1: q1 = vec(p1) + t1 * d1
        // L2: q2 = vec(p2) + t2 * d2
        // minimizing distance vector d = q2 - q1 = vec(p1, p2) + t2 * d2 - t1 *
        // d1 (d, d1) = 0 && (d, d2) = 0 (minimum d is perpendicular to d1 and
        // d2) solve system (u = point_vec = vec(p1, p2)) { (u, d1) = t1 * (d1,
        // d1) - t2 * (d1, d2) | u1 = t1 * d11 - t2 * d12 { (u, d2) = t1 * (d1,
        // d2) - t2 * (d2, d2) | u2 = t1 * d12 - t2 * d22
...
public:
    ...
    std::string to_string() const {
        return "Triangle(" + p1_.to_string() + ", " + p2_.to_string() + ", " +
               p3_.to_string() + ")";
    }
};
}; // namespace triangles