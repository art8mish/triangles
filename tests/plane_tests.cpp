...
TEST_F(TestPlane, Equality) {
    ...
    ASSERT_TRUE(plane_main == plane_main);

    plane_t plane_same{p_zero, p_valid1, p_valid2};
    ASSERT_TRUE(plane_main == plane_same);
    ASSERT_FALSE(plane_main != plane_same);
    ASSERT_TRUE((plane_main == plane_same) == (plane_same == plane_main));
    ASSERT_TRUE((plane_main != plane_same) == (plane_same != plane_main));
    ASSERT_TRUE(plane_main.normal().collinear_with(plane_same.normal()));

    vec_t vec_eq{0, 1, 0};
    plane_t plane_eq{p_zero, vec_eq};
    ASSERT_TRUE(plane_main == plane_eq);
    ASSERT_FALSE(plane_main != plane_eq);
    ASSERT_TRUE((plane_main == plane_eq) == (plane_eq == plane_main));
    ASSERT_TRUE((plane_main != plane_eq) == (plane_eq != plane_main));
    ASSERT_TRUE(plane_main.normal().collinear_with(plane_eq.normal()));

    point_t p_valid3{1, 1, 1};
    plane_t plane_non_eq{p_zero, p_valid1, p_valid3};
    ASSERT_FALSE(plane_main == plane_non_eq);
    ASSERT_TRUE(plane_main != plane_non_eq);
    ASSERT_TRUE((plane_main == plane_non_eq) == (plane_non_eq == plane_main));
    ASSERT_TRUE((plane_main != plane_non_eq) == (plane_non_eq != plane_main));
    ASSERT_FALSE(plane_main.normal().collinear_with(plane_non_eq.normal()));

    ASSERT_FALSE(plane_eq == plane_non_eq);
    ASSERT_TRUE(plane_eq != plane_non_eq);
    ASSERT_TRUE((plane_eq == plane_non_eq) == (plane_non_eq == plane_eq));
    ASSERT_TRUE((plane_eq != plane_non_eq) == (plane_non_eq != plane_eq));
...
TEST_F(TestPlane, Contains) {
    ...
    ASSERT_TRUE(plane.contains(p_zero));
    ASSERT_TRUE(plane.contains(p1));
    ASSERT_TRUE(plane.contains(p2));

    ASSERT_TRUE(plane.contains(point_t{0, 0, 10}));
    ASSERT_TRUE(plane.contains(point_t{5, 0, -5}));

    ASSERT_FALSE(plane.contains(point_t{0, 1, 0}));
...
TEST_F(TestPlane, ParallelPlanes) {
    ...
    ASSERT_TRUE(plane1.parallel_to(plane1));
    ASSERT_TRUE(plane1.normal().collinear_with(plane1.normal()));

    plane_t plane2{point_t{0, 2, 0}, point_t{1, 2, 0}, point_t{1, 2, 1}};
    ASSERT_TRUE(plane1.parallel_to(plane2));
    ASSERT_TRUE(plane2.parallel_to(plane1));
    ASSERT_TRUE(plane1.normal().collinear_with(plane2.normal()));

    plane_t plane3{p_zero, p1, point_t{1, 1, 1}};
    ASSERT_FALSE(plane3.parallel_to(plane1));
    ASSERT_FALSE(plane1.parallel_to(plane3));
    ASSERT_FALSE(plane3.normal().collinear_with(plane1.normal()));

    ASSERT_FALSE(plane3.parallel_to(plane2));
...
TEST_F(TestPlane, SignedDistance) {
    ...
    point_t p2{1, 0, 1};
    plane_t plane{p_zero, p1, p2};
    ASSERT_TRUE(zero<double>(plane.signed_distance(p_zero), eps_));
    ASSERT_TRUE(zero<double>(plane.signed_distance(p1), eps_));
    ASSERT_TRUE(zero<double>(plane.signed_distance(p2), eps_));

    ASSERT_TRUE(zero<double>(plane.signed_distance(point_t{5, 0, 10}), eps_));

    double dist1 = plane.signed_distance(point_t{1, 2, 1});
    double dist2 = plane.signed_distance(point_t{1, -2, 1});
    ASSERT_TRUE(equal<double>(std::abs(dist1), 2, eps_));
    ASSERT_TRUE(equal<double>(std::abs(dist2), 2, eps_));
    ASSERT_TRUE(equal<double>(dist1 * dist2, -4, eps_));
